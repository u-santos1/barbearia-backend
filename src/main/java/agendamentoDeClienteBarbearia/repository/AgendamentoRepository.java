...
""")
    ...
    @Query("SELECT COUNT(a) FROM Agendamento a WHERE a.barbeiro.email = :email AND a.status = 'CONFIRMADO' AND a.dataHoraInicio >= :inicioDia AND a.dataHoraInicio <= :fimDia")
    long contarConfirmadosHoje(@Param("inicioDia") LocalDateTime inicioDia,
                               @Param("fimDia") LocalDateTime fimDia,
                               @Param("email") String email);


    @Query("SELECT COUNT(a) FROM Agendamento a WHERE a.barbeiro.email = :email AND a.dataHoraInicio > :agora AND a.dataHoraInicio <= :amanha")
